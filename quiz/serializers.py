...
from .models import Quiz, QuizRuleType, QuizTag, QuizIOMode
...
class CreateQuizCodeTemplateSerializer(serializers.Serializer):
    pass
...
class BaseQuizSerializer(serializers.ModelSerializer):
    ...
    def get_public_template(self, obj):
        ...
        for lang, code in obj.template.items():
            ret[lang] = parse_quiz_template(code)["template"]
        return ret
...
class ExportQuizSerializer(serializers.ModelSerializer):
    ...
    def get_test_case_score(self, obj):
        return [{"score": item["score"] if obj.rule_type == QuizRuleType.OI else 100,
